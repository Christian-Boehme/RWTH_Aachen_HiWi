...
## Job name and files
#SBATCH --job-name=05_CH4_Air_Diffusion_TempGrad

## OUTPUT AND ERROR
#SBATCH -o job.05_CH4_Air_Diffusion_TempGrad.%j.out
#SBATCH -e job.05_CH4_Air_Diffusion_TempGrad.%j.err
...
args='-i 05_CH4_Air_Diffusion_TempGrad.input'

date
##$MPIEXEC $FLAGS_MPI_BATCH $I_MPI_PIN $I_MPI_PIN_DOMAIN ./$exe $args
##$MPIEXEC $FLAGS_MPI_BATCH 
./$exe $args 2>&1 | tee job_05_CH4_Air_Diffusion_TempGrad.out
date

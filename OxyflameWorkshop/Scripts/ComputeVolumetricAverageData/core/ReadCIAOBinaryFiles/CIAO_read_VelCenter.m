function [varTmp]=CIAO_read_VelCenter(filename,varname)
%
...
if (skip==512)
    ...
    dummy = fread(fileopen,((idata(4)-idata(3)+1)*(idata(6)-idata(5)+1)*(idata(8)-idata(7)+1)*(idata(10)-idata(9)+1)),'real*8');
    var_tmp=reshape(dummy,(idata(4)-idata(3)+1),(idata(6)-idata(5)+1),(idata(8)-idata(7)+1),(idata(10)-idata(9)+1));
    var=squeeze(var_tmp);
end
fclose(fileopen);

if     (strcmp('U',varname)==1)
    varTmp = 0.5*(var(1:end-1,1:end-1,1:end-1)+var(2:end,1:end-1,1:end-1));
elseif (strcmp('V',varname)==1)
    varTmp = 0.5*(var(1:end-1,1:end-1,1:end-1)+var(1:end-1,2:end,1:end-1))
elseif (strcmp('W',varname)==1)
    varTmp = 0.5*(var(1:end-1,1:end-1,1:end-1)+var(1:end-1,1:end-1,2:end));
end
...
# set data to one
df_fit = pd.DataFrame(list(zip(x_new, fitted_y_data)), columns=[col[0], 'rfrp'])
...
plt.plot(x_new, df_fit['rfrp'], 'x', label='fitted data')
plt.xlabel('time [s]')
plt.ylabel('$r_f/r_p$')
plt.title('{0}'.format((os.getcwd()).split('/')[-1]))
plt.legend(loc='best')
plt.tight_layout()
plt.savefig('SmoothedData_' + case + '.png')
#plt.show()

# df to csv
df_fit.to_csv('SmoothedData_' + case + '.txt', sep='\t', float_format='%12.6E', index=False)

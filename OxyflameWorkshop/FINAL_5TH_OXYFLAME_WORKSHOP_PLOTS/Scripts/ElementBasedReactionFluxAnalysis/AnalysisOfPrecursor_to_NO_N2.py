...
def main():
    ...
    for i in node_end:
        ...
        for node in precursors:
            ...
            if pathways:
                ...
                print(
                    'Global-product for {} -> {}: {:.5E}'.format(node, i, global_product))
    ...
    print('Contribution from {:7s} -> {}: {:.3f}%'.format(
        precursors[0], node_end[0], float(NH3[0]/sum(NH3))))
    print('Contribution from {:7s} -> {}: {:.3f}%'.format(
        precursors[1], node_end[0], float(HCN[0]/sum(HCN))))
    if sum(C5H5N) != 0:
        print('Contribution from {:7s} -> {}: {:.3f}%'.format(
            precursors[2], node_end[0], float(C5H5N[0]/sum(C5H5N))))
    print('Contribution from {:7s} -> {}: {:.3f}%'.format(
        precursors[0], node_end[1], float(NH3[1]/sum(NH3))))
    print('Contribution from {:7s} -> {}: {:.3f}%'.format(
        precursors[1], node_end[1], float(HCN[1]/sum(HCN))))
    if sum(C5H5N) != 0:
        print('Contribution from {:7s} -> {}: {:.3f}%'.format(
            precursors[2], node_end[1], float(C5H5N[1]/sum(C5H5N))))

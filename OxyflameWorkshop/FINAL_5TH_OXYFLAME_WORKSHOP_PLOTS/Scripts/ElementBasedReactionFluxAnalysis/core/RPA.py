        ...
        for j in reactions:
            ...
            if t in r:
                ...
                if reactant == '':
                    ...
                    reactant = reactant + ',' + \
                        ','.join([x for x in r if x != t])
        return reactant

    if len(reactions) == 0:
        sys.exit('\nCould not find reaction which produces ' +
...
def edges(tool, dfn, df, tail, lim, species, graph, open_nodes, closed_nodes, font, fsize, q, DTness, p_analysis):
...
def CreateGraphic(tool, target_spec, norm, df, ini, lim, spec, graph, f, fs, q, DTness, pa):

    graph, s_to_p, pspec = edges(
        tool, norm, df, ini, lim, spec, graph, [], [], f, fs, q, DTness, pa)
    if len(target_spec) != 0:
        s_to_p = list(set(s_to_p + target_spec))

    while len(s_to_p) != 0:
        graph, s_to_p, pspec = edges(
            tool, norm, df, s_to_p[0], lim, spec, graph, s_to_p, pspec, f, fs, q, DTness, pa)
        del s_to_p[0]

    return graph, pspec


def GenerateGraphic(df, target_spec, initial_species, limit, font, fontsize, cnode, q, NormTot, NormOutGoing, DThickness, single, odir, pa):
    ...
    graph, graph_nodes = CreateGraphic(
        'NETWORKX', spec_to_plot, df_n, df, initial_species, 0, species, graph, font, fontsize, q, DThickness, pa)
    ...
    # Flux normalization
    if NormTot:
        # sum of all element fluxes is 100 %
        nodes = graph.nodes()
        # absolute flux
        tot_weight = 0
        for i in nodes:
        ...
        #        graph.get_edge_data(e[0], e[1])['weight'])
        # renormalization
        for e in graph.in_edges(i):
            df_norm.iat[species.index(e[0]), species.index(e[1])] = float(
                # [%]
        ...
        if NormOutGoing is False:
            ...
            df_norm = ut.FluxNormalization(df_norm, False, 'Output', 'RPA')
    ...
    dot, dot_nodes = CreateGraphic('DOT', spec_to_plot, df_norm, df, initial_species,
                                   0, species, dot, font, fontsize, q, DThickness, pa)
    # detect_cycles(dot)
    ...
    if single:
        ...
        with open(odir + "ElementReactionFluxGraph.dot", "w") as out:
            for i in range(len(inp) - 1):
                ...
                if 'label=' in inp[i] and pa is False:
    ...
    os.system('dot -Tpng ' + odir + 'ElementReactionFluxGraph.dot -o ' +
              odir + 'ElementReactionFluxGraph.png')

    # create eps file of reaction pathway graph
    os.system('dot -Tps2 ' + odir + 'ElementReactionFluxGraph.dot -o ' +
              odir + 'ElementReactionFluxGraph.eps')

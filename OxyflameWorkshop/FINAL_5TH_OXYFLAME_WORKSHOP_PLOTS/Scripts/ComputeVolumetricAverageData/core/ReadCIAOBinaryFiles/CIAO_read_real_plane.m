function [var]=CIAO_read_real_plane(filename,varname)
%
...
if (skip==512)
    ...
    dummy = fread(fileopen,((idata(6)-idata(5)+1)*(idata(8)-idata(7)+1)),'real*8');
    var_tmp=reshape(dummy,(idata(6)-idata(5)+1),(idata(8)-idata(7)+1));
    var=squeeze(var_tmp);
end
fclose(fileopen);

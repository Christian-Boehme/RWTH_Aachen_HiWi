...
% => Tgas or YOH is used for fitting (curve-fitting)
% ================================
...
Fig_outpath = ['~/NHR/DE_Analysis/DNS_Data_PostProcessing/Figures/' ...
    'RegimeDiagram_FlameletFitting_TgasYOH/'];
DE_InputFiles = {'DEGridpointsZst_ign.csv', ...
    'DEGridpointsZst_OHmax.csv', ...
    'DEGridpointsZst_fullComb.csv',...
    'DEGridpointsZst_final.csv'};
%-------
% if flamelet fitting based on: Tgas
% DissipationElementFlameletData = {'DE_Flamelets_Tgas_PDF_ign.csv', ...
%     'DE_Flamelets_Tgas_PDF_OHmax.csv', ...
%     'DE_Flamelets_Tgas_PDF_fullComb.csv'};
% if flamelet fitting based on: YOH
DissipationElementFlameletData = {'DE_Flamelets_YOH_PDF_ign.csv', ...
    'DE_Flamelets_YOH_PDF_OHmax.csv', 'DE_Flamelets_YOH_PDF_fullComb.csv'};
%-------
...
if contains(DissipationElementFlameletData{c}, '_Tgas_')
    Fig_outpath = strcat(Fig_outpath, 'Tgas/');
    fprintf('Regime diagram based on Tgas-fitting!\n');
elseif contains(DissipationElementFlameletData{c}, '_YOH_')
    Fig_outpath = strcat(Fig_outpath, 'YOH/');
    fprintf('Regime diagram based on YOH-fitting!\n');
else
    fprintf('\nCant detect Tgas or YOH in input file!');
    return
end
...
for i = c:c
    ...
    % flamelet fitting
    de_flamelet_data = readtable(strcat(Data_path, ...
    ...
    for j = 1:DE_Zst
        ...
        Flamelet_Tinlet = de_flamelet_data.Tinlet(j);
        Flamelet_StrainRate = de_flamelet_data.StrainRate(j);
        FilteredFlamelts = FlameletData.Tinlet == Flamelet_Tinlet;
        [~, idx] = min(abs(FlameletData.StrainRate( ...
            FilteredFlamelts) - Flamelet_StrainRate));

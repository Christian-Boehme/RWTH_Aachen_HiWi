...
set out '../figures_postscripteps/Alzueta_C5H5N_Set7.eps'
...
# Experiments
f1 = '../ExperimentalData/Alzueta_C5H5N_Set7.txt'
# FM simulations
f2 = ARG1
f3 = ARG2
f4 = ARG3
...
y_max = STATS_max
stats f2 using (column("X-C5H5N")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f3 using (column("X-C5H5N")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f4 using (column("X-C5H5N")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
...
set ylabel 'X_{C_5H_5N} [ppm]' offset xpos,0

set label '{/Symbol l} = 6.1' at graph 0.03, graph 0.10

# create figure
p f1 u 1:($2*1E+06) notitle w p ps psizeA lw plw pt 7 lc rgb lcsymA\
 ,f2 u 2:((column('X-C5H5N'))*1E+06) notitle w l lt 1 lw lwrefA lc rgb lcrefA dashtype 2\
 ,f3 u 2:((column('X-C5H5N'))*1E+06) notitle w l lt 1 lw lwdetA lc rgb lcdetA dashtype 3\
 ,f4 u 2:((column('X-C5H5N'))*1E+06) notitle w l lt 1 lw lwnewA lc rgb lcnewA\

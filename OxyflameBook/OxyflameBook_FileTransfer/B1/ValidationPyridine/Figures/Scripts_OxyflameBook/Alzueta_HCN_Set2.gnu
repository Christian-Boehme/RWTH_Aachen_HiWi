...
set out '../figures_OxyflameBook/Alzueta_HCN_Set2.eps'
...
# Experiments
f1 = '../ExperimentalData/Alzueta_HCN_Set2.txt'
# FM simulations
f2 = ARG1
f3 = ARG2
f4 = ARG3
...
y_max = STATS_max
stats f2 using (column("X-HCN")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f3 using (column("X-HCN")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f4 using (column("X-HCN")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
...
p f1 u 1:($2*1E+06) notitle w p ps psizeA lw plw pt 7 lc rgb lcsymA\
 ,f2 u 2:((column('X-HCN'))*1E+06) notitle w l lt 1 lw lwrefA lc rgb lcrefA dashtype 2\
 ,f3 u 2:((column('X-HCN'))*1E+06) notitle w l lt 1 lw lwdetA lc rgb lcdetA dashtype 3\
 ,f4 u 2:((column('X-HCN'))*1E+06) notitle w l lt 1 lw lwnewA lc rgb lcnewA\

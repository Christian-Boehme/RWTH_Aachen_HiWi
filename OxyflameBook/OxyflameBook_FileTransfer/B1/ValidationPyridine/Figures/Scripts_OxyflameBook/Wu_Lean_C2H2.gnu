...
set out '../figures_OxyflameBook/Wu_Lean_C2H2.eps'
...
# Experiments
f1 = '../ExperimentalData/Wu_Lean_C2H2_MBMS.txt'
f2 = '../ExperimentalData/Wu_Lean_C2H2_GC.txt'
# FM simulations
f3 = ARG1
f4 = ARG2
f5 = ARG3
...
y_max = STATS_max
stats f2 using ($2*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f3 using (column("X-C2H2")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f4 using (column("X-C2H2")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f5 using (column("X-C2H2")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
...
set ylabel 'X_{C_2H_2} [ppm]' offset xpos,0

# create figures
p f1 u 1:($2*1E+06) notitle w p ps psizeWL lw plw pt 7 lc rgb lcsymWL\
 ,f2 u 1:($2*1E+06) notitle w p ps psizeWL lw plw pt 7 lc rgb lcsymWL_GC\
 ,f3 u 1:((column('X-C2H2'))*1E+06) notitle w l lt 1 lw lwrefWL lc rgb lcrefWL dashtype 2\
 ,f4 u 1:((column('X-C2H2'))*1E+06) notitle w l lt 1 lw lwdetWL lc rgb lcdetWL dashtype 3\
 ,f5 u 1:((column('X-C2H2'))*1E+06) notitle w l lt 1 lw lwnewWL lc rgb lcnewWL\

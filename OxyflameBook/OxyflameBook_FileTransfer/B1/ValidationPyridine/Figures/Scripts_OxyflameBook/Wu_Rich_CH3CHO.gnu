...
set out '../figures_OxyflameBook/Wu_Rich_CH3CHO.eps'
...
# Experiments
f1 = '../ExperimentalData/Wu_Rich_CH3CHO.txt'
# FM simulations
f3 = ARG1
f4 = ARG2
f5 = ARG3
...
y_max = STATS_max
stats f3 using (column("X-CH3CHO")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f4 using (column("X-CH3CHO")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
stats f5 using (column("X-CH3CHO")*1E+06) nooutput
if (STATS_max > y_max) {y_max = STATS_max}
...
set ylabel 'X_{CH_3CHO} [ppm]' offset xpos,0

# create figures
p f1 u 1:($2*1E+00) notitle w p ps psizeWR lw plw pt 7 lc rgb lcsymWR\
 ,f3 u 1:((column('X-CH3CHO'))*1E+06) notitle w l lt 1 lw lwrefWR lc rgb lcrefWR dashtype 2\
 ,f4 u 1:((column('X-CH3CHO'))*1E+06) notitle w l lt 1 lw lwdetWR lc rgb lcdetWR dashtype 3\
 ,f5 u 1:((column('X-CH3CHO'))*1E+06) notitle w l lt 1 lw lwnewWR lc rgb lcnewWR\

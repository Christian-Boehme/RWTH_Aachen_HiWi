...
def CreateFigure(cases, limit, outdir, f1, f2, f3, f4, f5, f6, label1, label2, label3, label4, label5, label6, legend_pos, reac_color, x_label, species, cons, prod, fs):
    ...
    # empty global df
    df_complete = pd.DataFrame(float(0.0), columns=[
                               'Reactions', 'Case1', 'Case2', 'Case3', 'Case4', 'Case5', 'Case6'], index=np.arange(len(reac)))
...
#        xmax = max(max(Data1), max(Data2), max(Data3), max(Data4), max(Data5))
        ...
        xmin = min(min(Data1), min(Data2), min(Data3),
                   min(Data4), min(Data5), min(Data6))
        xmax = max(max(Data1), max(Data2), max(Data3),
                   max(Data4), max(Data5), max(Data6))
    ...
    for i in range(len(reactions)):
        ...
        reactants, products, a, b = DetermineReactantAndProductSpecies(
            reactions[i], Data1[i], species)
        if '+M' in reactions[i]:
            reactants[-1] += '(+M)'
            products[-1] += '(+M)'
    ...
    if cons:
        plt.savefig(outdir + '/ROPA_' + species + '_cons_FontSize_' + str(fs) + '.pdf',
                    format="pdf", bbox_inches='tight')
    elif prod:
        plt.savefig(outdir + '/ROPA_' + species + '_prod_FontSize_' + str(fs) + '.pdf',
                    format="pdf", bbox_inches='tight')
    else:
...
def main(args=None):
    ...
    parser.add_argument('-i2', type=str, required=False,
                        help='Second input file.')
    parser.add_argument('-i3', type=str, required=False,
                        help='Third input file.')
    parser.add_argument('-i4', type=str, required=False,
                        help='Fourth input file.')
    parser.add_argument('-i5', type=str, required=False,
                        help='Fifth input file.')
    parser.add_argument('-i6', type=str, required=False,
                        help='Sixth input file.')
    parser.add_argument('-lim', type=float, required=False,
                        help='Limit/Tolerance for the ROPA.')
    parser.add_argument('-outdir', type=str, required=True,
                        help='Name of output - directory.')
    parser.add_argument('-l1', type=str, required=False,
                        help='Label for input file one.')
    parser.add_argument('-l2', type=str, required=False,
                        help='Label for input file two.')
    parser.add_argument('-l3', type=str, required=False,
                        help='Label for input file three.')
    parser.add_argument('-l4', type=str, required=False,
                        help='Label for input file four.')
    parser.add_argument('-l5', type=str, required=False,
                        help='Label for input file five.')
    parser.add_argument('-l6', type=str, required=False,
                        help='Label for input file six.')
    parser.add_argument('-legend_pos', type=str,
                        required=False, help='Legend position.')
    ...
    parser.add_argument('-fs', type=int, required=False,
                        help='Fontsize of figure. Default: 20')
    ...
    if args.i6 != '':
        cases += 1
    CreateFigure(cases, args.lim, args.outdir, args.i1, args.i2, args.i3, args.i4, args.i5, args.i6, args.l1, args.l2, args.l3,
                 args.l4, args.l5, args.l6, args.legend_pos, args.rcs, args.xlabel, args.species, args.cons, args.prod, args.fs)

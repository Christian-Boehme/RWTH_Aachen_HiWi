...
def GeneratePlot(f, fs, df, df_2, df_3, df_4, species, limit, outdir, label1, label2, label3, label4, legend_pos, reac_color, x_label, Nreac, cons, prod):
    ...
    if cases == 1:
        ...
        for i in range(len(Data1)):
            if Data1[i] >= 0:
                plt.barh(ind[i], Data1[i], width, color='#1f78b4')
            else:
                plt.barh(ind[i], Data1[i], width, color='#e31a1c')
        xmin = min(Data1)
        xmax = max(Data1)
    elif cases == 2:
        bwidth = width * 2 * 0.85
        bar1 = plt.barh(ind+width, Data1, bwidth,
                        color='#1f78b4', edgecolor='white', linewidth=1)
        bar2 = plt.barh(ind-width, Data2, bwidth,
                        color='#e31a1c', edgecolor='white', linewidth=1)
        ...
        bar2 = plt.barh(ind, Data2, bwidth, color='#e31a1c',
                        edgecolor='white', linewidth=1)
        bar3 = plt.barh(ind-width, Data3, bwidth,
                        color='#33a02c', edgecolor='white', linewidth=1)
    ...
    for i in range(len(reactions)):
        ...
        if r[0] in reac_color.keys():
            ...
            reactions[i] = r'$\Longrightarrow$' + reactions[i]
            #pos = color[i].get_position()
            #print(int(pos[0]))
            #print(bwidth)
            #a = plt.text(int(pos[0]) - abs(xmin), int(pos[1]) - 0.25, r'$\Longrightarrow$', color='red')
            #b = plt.annotate(reactions[i], color='green', xy=(1,0))
            #reactions[i] = a + reactions[i]
            #reactions[i] = r'\textcolor{#e13a1c}{HERE}' + reactions[i]

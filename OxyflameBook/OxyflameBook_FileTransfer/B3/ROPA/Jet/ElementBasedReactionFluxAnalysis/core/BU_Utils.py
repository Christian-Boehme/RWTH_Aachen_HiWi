...
def ElementContainingReactions(nr, elements, idx):
    ...
    for i in nr:
        ...
        species = ls + rs
        for j in species:
            if elements[j].iloc[idx] != 0: #elements[j][idx] != 0:
                df_ele.loc[0, i] = True #df_ele[i][0] = True
                break
...
def TimeIntegratedNetReactionRate(df, df_inp):
    ...
    for i in df.columns:
        df_int.loc[0, i] = float(integrate.simpson(df[i].to_list(), x=time)) #df_int[i][0] = float(integrate.simpson(df[i].to_list(), x=time))

    # integrated average net reaction rate
    endt = time[-1]
    for i in df.columns:
        df_int.loc[0, i] = float(df_int.loc[0, i] / endt) #df_int[i][0] = float(df_int[i][0] / endt)
...
def DetermineReactantAndProductSpecies(r, W):
    ...
    if W.loc[0, r] < 0: #W.loc['ReactionRate', r] < 0: #W[r][0] < 0:
...
def FilterSpecies(species, coeff, idx, ele):
    ...
    for i in range(len(species)):
        if ele.iloc[idx, i] != 0: #ele[species[i]][idx] != 0:
            s.append(species[i])
            c.append(coeff[i])
...
def SpeciesMassFlux(sl, nr, element, idx, MM_ele, df_ele, ele_in_reac, W_net, save, subdir):
    ...
    for i in range(len(sl)): #for i in sl:
        if df_ele.iloc[idx, i] != 0: #df_ele[i][idx] != 0:
    ...
    for j in nr:
        if ele_in_reac[j][0]:
            ...
            ele_reactants, coeff_reac = FilterSpecies(
                reactants, coeff_reac, idx, df_ele)
            ele_products, coeff_prod = FilterSpecies(
                products, coeff_prod, idx, df_ele)
            tot_N = float(
                sum([df_ele.loc[idx, x] * coeff_reac[ele_reactants.index(x)] for x in ele_reactants]))
                #sum([df_ele[x][idx] * coeff_reac[ele_reactants.index(x)] for x in ele_reactants]))
            #print('\n\nReaction: {0}\nreactants: {1}\nproducts: {2}\ncoeff_reac: {3}\ncoeff_prod: {4}\ntot_N: {5}'.format(j, ele_reactants, ele_products, coeff_reac, coeff_prod, tot_N))
            for rs_idx in range(len(ele_reactants)):
                #for rs in ele_reactants:
                rs = ele_reactants[rs_idx]
                cr = coeff_reac[ele_reactants.index(rs)]
                n_r = df_ele.iloc[idx, rs_idx] #df_ele[rs][idx]
                for ps in ele_products:
                    cp = coeff_prod[ele_products.index(ps)]
                    n_p = df_ele[ps][idx] * cp
                    if rs == ps and cr == cp:
                        continue
                    # compute flux according to X.Wen
                    #flux = n_r * n_p * MM_ele * cr * W_net[j][0] / tot_N
                    # compute flux according to A.Shamooni
                    flux = n_r * n_p / tot_N * W_net[j][0]

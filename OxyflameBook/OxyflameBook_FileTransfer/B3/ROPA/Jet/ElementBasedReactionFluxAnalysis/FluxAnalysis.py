...
def ComputeElementFluxes(element, df, start_time, end_time, save, outdir, species):
    ...
    if save:
        ...
        W.to_csv(outdir + 'NetReactionRates.csv', sep='\t', float_format='%20.6E', index=False)
...
def main(args=None):
    ...
    parser.add_argument('-PRates_1', type=str, required=False,
                        help='Plot production or destruction rates. NOTE: use flag -species ALL to plot every rate.')
    ...
    if args.i != '':
        ...
        rpa.GenerateGraphic(L_RPA, args.target, args.species, args.limit, args.f,
                            args.fs, args.NodeColor, q, args.NormOutGoing, args.DThickness, args.single, outdir, args.PrecursorAnalysis)
